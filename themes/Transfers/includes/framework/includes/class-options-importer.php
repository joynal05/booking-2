...
class OF_Import_Export {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'transfers' ) );
	}
	...
	public function add_options( $options ){
		$options[] = array(
			'name' => __( 'Import/Export', 'transfers' ),
			'type' => 'heading'
		);
		$options[] = array(
			'name' => __( 'Import Settings', 'transfers' ),
			'id' => 'import_settings',
			'rows' => 10,
			'type' => 'import'
	     );
		$options[] = array(
			'name' => __( 'Export Settings', 'transfers' ),
			'desc' => __( 'Select all and copy to export your settings.', 'transfers'  ),
			'id' => 'export_settings',
			'type' => 'export'
	     );
		return $options;
    ...
    public function import_option_type( $option_name, $option, $values ){
		...
		$desc = __( 'Paste your exported settings here. When you click "Import" your settings will be imported to this site.', 'transfers'  );
		$value = esc_attr__( 'Import', 'transfers' );
		$msg = esc_js( __( 'Click OK to import. All current theme settings will be overwritten!', 'transfers' ) );
    ...
    public function export_option_type( $option_name, $option, $values ){
		...
		if ( $this->theme_options && is_array( $this->theme_options ) ) {
			...
			$this->theme_options['theme-type'] = 'transfers';
			$this->theme_options['theme-version'] = TRANSFERS_VERSION;

			// Generate the export data.
			$val = base64_encode( maybe_serialize( (array)$this->theme_options ) );
		} else {
			$val = __( 'ERROR! You don\'t have any options to export. Trying saving your options first.', 'transfers' );
		}
    ...
    public function import_settings( $input ){
		...
		if ( isset( $input['import_settings'] ) && trim( $input['import_settings'] ) !== '' ){ 
			...
			if ( is_array( $data ) &&
				(
					(isset( $data['theme-type'] ) && 'transfers' == $data['theme-type'] && isset( $data['theme-version'] ) && $data['theme-version'] == TRANSFERS_VERSION) ||
	...
	public function add_save_notice(){ 
		...
		if( $success ){
			...
			if( $success === 'success' ) {
				add_settings_error( 'options-framework', 'import_options', __( 'Options imported.', 'transfers' ), 'updated fade' );	
			} else {
				add_settings_error( 'options-framework', 'import_options_fail', __( 'Options could not be imported.', 'transfers' ), 'error' );
			}
		}
